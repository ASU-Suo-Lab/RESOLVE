...
class AnchorHeadMulti(AnchorHeadTemplate):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range,
        ...
        if self.use_multihead:
            self.build_class_anchor_indices()
    ...
    def make_multihead(self, input_channels):
        ...
        for rpn_head_cfg in rpn_head_cfgs:
            num_anchors_per_location = sum([self.num_anchors_per_location[self.class_names.index(head_cls)]
    ...
    def get_cls_layer_loss(self):
        ...
        one_hot_targets = one_hot_targets[..., 1:] # (B, N, num_class)

        cls_losses = 0

        for idx, cls_pred in enumerate(cls_preds):
            head_classes = self.model_cfg.RPN_HEAD_CFGS[idx]['HEAD_CLS_NAME']  # e.g. ['truck','barrier']
            cur_num_class = self.rpn_heads[idx].num_class                      # local class count

            # build global anchor indices for this head (concat class blocks in head_classes order)
            idx_list = []
            for name in head_classes:
                name = str(name)
                s = self.class_anchor_start[name]
                n = self.class_anchor_count[name]
                idx_list.append(torch.arange(s, s + n, device=box_cls_labels.device))
            head_anchor_idx = torch.cat(idx_list, dim=0)  # (A_h,)

            # gather labels/weights/onehot by anchor indices
            head_cls_weight = cls_weights.index_select(1, head_anchor_idx)                # (B, A_h)
            head_onehot_all = one_hot_targets.index_select(1, head_anchor_idx)            # (B, A_h, num_class)

            # select correct class columns for this head (NOT contiguous!)
            cols = (self.rpn_heads[idx].head_label_indices - 1).long().to(one_hot_targets.device)  # 0-based cols
            head_onehot = head_onehot_all.index_select(dim=2, index=cols)                 # (B, A_h, cur_num_class)

            # reshape pred
            cls_pred = cls_pred.view(batch_size, -1, cur_num_class)
            assert cls_pred.shape[1] == head_anchor_idx.numel(), \
                f"Head{idx}: pred anchors {cls_pred.shape[1]} != gathered {head_anchor_idx.numel()}"

   
            cls_loss_src = self.cls_loss_func(cls_pred, head_onehot, weights=head_cls_weight)  # [N, M]
    ...
    def get_box_reg_layer_loss(self):
        ...
        box_reg_targets = self.forward_ret_dict['box_reg_targets'] # (B, N, code)
        box_cls_labels = self.forward_ret_dict['box_cls_labels']  
        ...
        # build full flattened anchors in the SAME global order as targets (class blocks)
        # this matches AxisAlignedTargetAssigner(use_multihead=True) cat order
        anchors_full = torch.cat(
            [a.permute(3, 4, 0, 1, 2, 5).contiguous().view(-1, a.shape[-1]) for a in self.anchors],
            dim=0
        )  # (N, code)
        anchors_full = anchors_full.view(1, -1, anchors_full.shape[-1]).repeat(batch_size, 1, 1)  # (B, N, code)

        # if direction classifier enabled, precompute full dir targets once then gather per head
        dir_targets_full = None
        if box_dir_cls_preds is not None:
            dir_targets_full = self.get_direction_target(
                anchors_full, box_reg_targets,
                dir_offset=self.model_cfg.DIR_OFFSET,
                num_bins=self.model_cfg.NUM_DIR_BINS
            )  # (B, N, num_bins) one-hot

        box_losses = 0.0
        tb_dict = {}

        for idx, box_pred in enumerate(box_preds):
            head_classes = self.model_cfg.RPN_HEAD_CFGS[idx]['HEAD_CLS_NAME']
    
            # build global anchor indices for this head
            idx_list = []
            for name in head_classes:
                name = str(name)
                s = self.class_anchor_start[name]
            ...
            # gather targets/weights
            head_reg_target = box_reg_targets.index_select(1, head_anchor_idx)  # (B, A_h, code)
            head_reg_weight = reg_weights.index_select(1, head_anchor_idx)      # (B, A_h)
	    
            # reshape pred
            box_pred = box_pred.reshape(batch_size, -1, box_pred.shape[-1])

            # In your use_multihead=True + separate_reg_config, box_pred is already (B, A_h, code_size)
            # so above usually becomes (B, -1, code_size). Keep assertion:
            assert box_pred.shape[1] == head_anchor_idx.numel(), \
                f"Head{idx}: pred anchors {box_pred.shape[1]} != gathered {head_anchor_idx.numel()}"


            # sin(a - b) = sinacosb-cosasinb
            if box_dir_cls_preds is not None:
                box_pred_sin, reg_target_sin = self.add_sin_difference(box_pred, head_reg_target)
                loc_loss_src = self.reg_loss_func(box_pred_sin, reg_target_sin, weights=head_reg_weight)  # [N, M]
            else:
                loc_loss_src = self.reg_loss_func(box_pred, head_reg_target, weights=head_reg_weight)  # [N, M]
            ...
            if box_dir_cls_preds is not None:
                ...
                head_dir_logits = box_dir_cls_preds[idx].view(batch_size, -1, self.model_cfg.NUM_DIR_BINS)
                assert head_dir_logits.shape[1] == head_anchor_idx.numel()
                head_dir_target = dir_targets_full.index_select(1, head_anchor_idx)  # (B, A_h, num_bins)
                
                # weights: positives only (gathered)
                head_pos = positives.index_select(1, head_anchor_idx).type_as(head_dir_logits)  # (B, A_h)
                w = head_pos / torch.clamp(head_pos.sum(-1, keepdim=True), min=1.0)

                dir_loss = self.dir_loss_func(head_dir_logits, head_dir_target, weights=w)
    ...
    def build_class_anchor_indices(self):
        """
           Build mapping: class_name -> (start_idx, count) in the global flattened anchor order
           used by AxisAlignedTargetAssigner when use_multihead=True.
        """
        assert self.use_multihead, "This mapping is for USE_MULTIHEAD=True"

        self.class_anchor_start = {}
        self.class_anchor_count = {}
        start = 0

        # target_assigner.anchor_class_names == ANCHOR_GENERATOR_CONFIG order
        for cls_name, anchors in zip(self.target_assigner.anchor_class_names, self.anchors):
            # anchors is the original multi-dim tensor for that class
            # target_assigner flattens by: permute(3,4,0,1,2,5).view(-1, code)
            n = anchors.permute(3, 4, 0, 1, 2, 5).contiguous().view(-1, anchors.shape[-1]).shape[0]

            self.class_anchor_start[str(cls_name)] = start
            self.class_anchor_count[str(cls_name)] = n
            start += n

        self.total_anchors = start
