...
from train_utils.train_utils import train_model,freeze_lidar_branch,register_grad_stride_debug
...
def main():
    ...
    if getattr(cfg.MODEL, 'FREEZE_LIDAR', False):
    ...
    # 只在主进程打印，避免刷屏
    if cfg.LOCAL_RANK == 0:
        register_grad_stride_debug(model, logger=logger)
    ...
    if dist_train and dist.is_available() and dist.is_initialized():
        # 确保所有进程都执行到这里再退出
