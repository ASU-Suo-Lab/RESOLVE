...
def load_gt_from_pkl(data_root, box_cls,val_name) -> EvalBoxes:
    all_annotations = EvalBoxes()
    # Load annotations and filter predictions and annotations.
    data_path = os.path.join(data_root, val_name[0])
...
def evaluate(root_path, output_dir, result_path, cfg, val_name):
    pred_boxes, meta = load_prediction(result_path, cfg.max_boxes_per_sample, DetectionBox)
    # TODO DSZ poly dataset
    gt_boxes = load_gt_from_pkl(root_path, DetectionBox,val_name)
    # Run evaluation.
    ...
    # ===== 新增：分距离桶评估 =====
    bin_results = cal_metrics_by_distance_bins(cfg, gt_boxes, pred_boxes,
                                                   bins=((0,50),(50,70),(70,float('inf'))))
    # 保存每个桶
    for bin_name, data in bin_results.items():
        with open(os.path.join(output_dir, f'metrics_summary_{bin_name}.json'), 'w') as f:
            json.dump(data['metrics_summary'], f, indent=2)
...
def cal_metrics_by_distance_bins(cfg, gt_boxes, pred_boxes,
    ...
    for lo, hi in bins:
        ...
        if len(gt_view.all) == 0:
            empty = DetectionMetrics(cfg).serialize()
            empty["note"] = "No GT in this distance bin."
            out[name] = {'metrics_summary': empty, 'metric_data_list': DetectionMetricDataList()}
            continue
