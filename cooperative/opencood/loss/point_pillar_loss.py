...
class PointPillarLoss(nn.Module):
    def __init__(self, args):
        ...
        self.use_dir = args.get('use_dir', False)
        self.dir_weight = args.get('dir_weight', 0.2)
        self.loss_dict = {}
    ...
    def forward(self, output_dict, target_dict):
        ...
        dir_loss = rm.new_tensor(0.0)
        if self.use_dir and 'dir_cls_preds' in output_dict and 'anchor_box' in output_dict:
            dir_targets = get_direction_target(targets, output_dict['anchor_box'])
            dir_logits = output_dict['dir_cls_preds'].permute(0, 2, 3, 1).contiguous().view(rm.shape[0], -1, 2)
            dir_weights = positives.type_as(dir_logits)
            dir_weights /= torch.clamp(dir_weights.sum(-1, keepdim=True), min=1.0)
            dir_loss_src = softmax_cross_entropy_with_logits(dir_logits.view(-1, 2),
                                                             dir_targets.view(-1, 2))
            dir_loss = dir_loss_src.view(dir_weights.shape) * dir_weights
            dir_loss = dir_loss.sum() / rm.shape[0]
            dir_loss *= self.dir_weight
...
def get_direction_target(reg_targets, anchors, dir_offset=0.0):
    batch_size = reg_targets.shape[0]
    anchors = anchors.view(1, -1, anchors.shape[-1]).repeat(batch_size, 1, 1)
    rot_gt = reg_targets[..., -1] + anchors[..., -1]
    dir_cls_targets = ((rot_gt - dir_offset) > 0).long()
    tensor_onehot = torch.zeros(*list(dir_cls_targets.shape), 2,
                                dtype=anchors.dtype, device=anchors.device)
    tensor_onehot.scatter_(-1, dir_cls_targets.unsqueeze(dim=-1), 1.0)
    return tensor_onehot


def softmax_cross_entropy_with_logits(logits, labels):
    logits = logits.permute(0, 1) if logits.ndim == 2 else logits
    loss_ftor = torch.nn.CrossEntropyLoss(reduction="none")
    return loss_ftor(logits, labels.max(dim=-1)[1])
