...
def train_one_epoch(model, optimizer, train_loader, model_func, lr_scheduler, accumulated_iter, optim_cfg,
    ...
    for cur_it in range(start_it, total_it_each_epoch):
        ...
        if cfg is not None and cfg.MODEL.get('FREEZE_LIDAR', False):
            freeze_lidar_branch(model)
            
            # ---- print once per epoch to verify ----
            if rank == 0 and logger is not None and cur_it == start_it:
                m = model.module if hasattr(model, "module") else model
                num_train = sum(p.numel() for p in m.parameters() if p.requires_grad)
                num_all = sum(p.numel() for p in m.parameters())
                logger.info(f"[E{cur_epoch}] Trainable params: {num_train}/{num_all}")
                logger.info(f"[E{cur_epoch}] lidar BN training? {m.backbone_3d.conv_input[1].training}")
                # 可选：确认 backbone_3d 确实没有可训练参数
                lidar_trainable = sum(p.numel() for p in m.backbone_3d.parameters() if p.requires_grad)
                logger.info(f"[E{cur_epoch}] backbone_3d trainable params: {lidar_trainable}")
...
def freeze_lidar_branch(model):
    """
    Freeze lidar branch in BevFusion: vfe/backbone_3d/map_to_bev_module/pfe
    Also keep them in eval() to stop SyncBN running stats updates.
    Works for both normal model and DDP-wrapped model.
    """
    import torch.nn as nn

    m = model.module if hasattr(model, "module") else model
    lidar_names = ['vfe', 'backbone_3d', 'map_to_bev_module', 'pfe']
    ...
    for name in lidar_names:
        if hasattr(m, name) and getattr(m, name) is not None:
            ...
            # 2) freeze behavior
            sub.eval()
            for s in sub.modules():
                if isinstance(s, (nn.BatchNorm1d, nn.BatchNorm2d, nn.BatchNorm3d, nn.SyncBatchNorm)):
                    s.eval()
                    
def register_grad_stride_debug(ddp_model, logger=None):
        ...
        for name, p in m.named_parameters():
            ...
            def make_hook(nm, param):
                def hook(grad):
                    if grad is None:
                        return grad
                    if (not grad.is_contiguous()) or (grad.stride() != param.stride()):
                        msg = (
                            f"[DDP-STRIDE] {nm} "
                            f"param.shape={tuple(param.shape)} param.stride={param.stride()} "
                            f"grad.shape={tuple(grad.shape)} grad.stride={grad.stride()} "
                            f"grad.contig={grad.is_contiguous()}"
                        )
