...
class Base3DTracker:
    ...
    def predict_before_associate(self, trajs):
        for tr in trajs:
            tr.predict()
    ...
    def track_single_frame(self, frame_info):
        ...
        """
        t0 = time.time()
        all_trajs_list = self.get_trajectory_bbox(self.all_trajs)
        MAX_TRAJS = self.cfg.get("MAX_TRAJS_FOR_MATCH", 300)
        if len(all_trajs_list) > MAX_TRAJS:
            all_trajs_list.sort(key=lambda t: float(t.bboxes[-1].det_score), reverse=True)
            keep = all_trajs_list[:MAX_TRAJS]
            drop = all_trajs_list[MAX_TRAJS:]
            trajs = keep

            # 关键：没参加匹配的轨迹也要“走时间”
            for tr in drop:
                self.all_trajs[tr.track_id].unmatch_update(frame_info.frame_id)
        else:
            trajs = all_trajs_list
        

        self.predict_before_associate(trajs)
        ...
        t2 = time.time()
        if match_res is None or len(match_res) == 0:
            match_res = np.zeros((0, 2), dtype=np.int64)
        matched_det_indices = set(match_res[:, 1].tolist()) if match_res.size else set()
        ...
        INIT_TH = self.cfg.get("NEW_TRACK_TH", 0.3)  # 你自己定
        for i in unmatched_det_indices:
            if init_bboxes[i].det_score < INIT_TH:
                continue
        ...
        print(f"[PROF] fid={frame_info.frame_id:4d} "
                  f"pred={(t1-t0):.3f}s match={(t2-t1):.3f}s rest={(t3-t2):.3f}s "
                  f"pool={len(self.all_trajs)} trajs={len(trajs)} dets={len(frame_info.bboxes)}")
