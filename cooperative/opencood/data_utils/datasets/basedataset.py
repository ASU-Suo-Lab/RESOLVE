...
class BaseDataset(Dataset):
    ...
    def __init__(self, params, visualize, train=True):
        ...
        self.data_augmentor = DataAugmentor(params.get('data_augment', []),
                                            train)
    ...
    def collate_batch_train(self, batch):
        ...
        for i in range(len(batch)):
            ...
            if anchor_box is None and ego_dict.get('anchor_box', None) is not None:
                anchor_box = ego_dict['anchor_box']
        ...
        if anchor_box is not None:
            output_dict['ego'].update({'anchor_box': torch.from_numpy(np.array(anchor_box))})
        if self.visualize:
            origin_lidar = \
