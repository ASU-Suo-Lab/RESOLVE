...
class TransFusionHead(nn.Module):
    ...
    def __init__(
        ...
        if self.query_local:
            ...
            self.query_r_coor_x, self.query_r_coor_y = torch.meshgrid(self.query_range, self.query_range, indexing="ij") 
    ...
    def predict(self, inputs):
        ...
        assert top_proposals_index.max() < bev_pos.shape[1], \
   		f"top idx {top_proposals_index.max().item()} >= bev_pos HW {bev_pos.shape[1]}"
    ...
    def get_bboxes(self, preds_dicts):
        ...
        if self.dataset_name == "nuScenes" or "sunlakes":
            self.tasks = [
                dict(num_class=9, class_names=[], indices=[0, 1, 2, 3, 4, 5, 6, 7, 9], radius=-1),
