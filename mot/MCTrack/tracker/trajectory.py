...
class Trajectory:
    ...
    def unmatch_update(self, frame_id,already_predicted=False):
        ...
        if not already_predicted:
            ...
            # 直接用上一次 predict 后的 state（你需要在 predict() 里缓存）
            predict_state = self.kalman_filter_pose.x.copy()
            predict_yaw = self.kalman_filter_yaw.x.copy()
            predict_size = self.kalman_filter_size.x.copy()
        # if self.cfg["IS_RV_MATCHING"]:
        #     predict_rvbox = self.kalman_filter_rvbox.predict()
